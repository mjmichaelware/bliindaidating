...
async def generate_and_create_dummy_user():
    """
    Generates a new dummy user with a random name, email, and an AI-generated profile summary.
    """
    try:
        ...
        prompt = f"""
        Write a short, engaging dating profile summary (2-3 sentences) for a person named {username}. 
        The summary should be fun and friendly, but do not mention a name.
        """
        
        # 3. Call the Gemini API to get the profile summary
        ai_summary = await call_gemini_api(prompt)

        # 4. Create a new user object with the generated data
        new_user = User(
            username=username,
            email=email,
        ...
        # 5. Add the new user to our in-memory database
...
async def generate_and_create_news_feed_post():
    ...
    try:
        ...
        # 2. Define the prompt for the Gemini API
        prompt = f"""
        Generate a single short and lighthearted news feed post (1-2 sentences) from the perspective of a user named {random_user.username}. 
        The post should be about something a person might do on a dating app, like a fun date or a hobby.
        """
