...
class UserProfile {
  ...
  final String? locationZipCode; // Changed to nullable
  // REMOVED: location_city and location_state are not explicit fields here
  ...
  final List<String>? languagesSpoken; // Made nullable
  ...
  final List<String>? hobbiesAndInterests; // Made nullable
  @JsonKey(name: 'love_languages')
  final List<String>? loveLanguages; // Made nullable
  @JsonKey(name: 'favorite_media')
  final List<String>? favoriteMedia; // Made nullable
  @JsonKey(name: 'marital_status')
  final String? maritalStatus;
  @JsonKey(name: 'has_children')
  final bool? hasChildren;
  @JsonKey(name: 'wants_children')
  final bool? wantsChildren;
  @JsonKey(name: 'relationship_goals')
  final String? relationshipGoals;
  final List<String>? dealbreakers; // Made nullable
  ...
  final List<String>? personalityTraits; // Made nullable
  // ADDED: This field was missing based on your error "The getter 'questionnaireAnswers' isn't defined"
  @JsonKey(name: 'questionnaire_answers')
  final Map<String, dynamic>? questionnaireAnswers;
  // RENAMED AND ADDED: This field was causing "personalityAssessmentResults" error
  ...
  UserProfile({
    ...
    // ADDED these parameters to the constructor to match the errors
    ...
    this.createdAt,
    this.updatedAt,
    // Deprecated fields in constructor
    this.fullName,
    this.gender,
    this.addressZip,
    this.interests,
  ...
  factory UserProfile.fromJson(Map<String, dynamic> json) {
    ...
    // The key here is to pass only the *actual* fields of UserProfile to the generated fromJson method.
    // The generated code will expect values for the fields defined in the UserProfile class.
    // Since location_city and location_state are NOT UserProfile fields, they should NOT be in the map
    // passed to _$UserProfileFromJson.
    return _$UserProfileFromJson({
      ...
      'height_cm': heightCm,
      // Ensure these are NOT passed to _$UserProfileFromJson if they are not actual fields
      // as _$UserProfileFromJson will try to map them to non-existent fields.
      // This is often implicitly handled if they are not defined in the class,
      // but explicitly removing them from the input map is safer.
      // If your JSON *does* contain them but you want them ignored, then JsonKey(ignore: true)
      // on a dummy field is another pattern, but simpler to just not have the field at all.
    });
  }
  ...
  Map<String, dynamic> toJson() {
    ...
    if (fullLegalName != null && json['full_name'] == fullLegalName) {
      json.remove('full_name');
    }
    if (genderIdentity != null && json['gender'] == genderIdentity) {
      json.remove('gender');
    }
    if (locationZipCode != null && json['address_zip'] == locationZipCode) {
      json.remove('address_zip');
    }
    // This comparison for lists needs to be careful, checking set equality is better
    // or just always remove if the old field might exist.
    ...
    if (heightCm != null && json['height'] == heightCm) {
      json.remove('height');
    } else if (json.containsKey('height')) {
      json.remove('height');
    }

    // Since location_city and location_state are not fields, they won't be in the map
    // generated by _$UserProfileToJson. These `.remove` calls are actually redundant
    // if the fields are not defined in the class, but harmless.
  ...
  UserProfile copyWith({
    ...
    // REMOVED locationCity and locationState from copyWith parameters
    ...
    // ADDED these to copyWith parameters
    ...
    DateTime? createdAt,
    DateTime? updatedAt,
    // Deprecated fields in copyWith (can be removed after migration)
    String? fullName,
    String? gender,
    String? addressZip,
  ...
  factory UserProfile.fromSupabaseUser(User user) {
    return UserProfile(
      userId: user.id,
      email: user.email ?? 'no-email@example.com', // Provide a default or handle null email
      ...
      updatedAt: user.updatedAt != null
          ...
          : null,
      isPhase1Complete: false, // Default to false for new profiles
      isPhase2Complete: false, // Default to false for new profiles
      agreedToTerms: false,
      agreedToCommunityGuidelines: false,
      // Initialize other fields to null or appropriate defaults
      fullLegalName: null,
      displayName: null,
      ...
      bio: null,
      lookingFor: null,
      hobbiesAndInterests: null, // Use null for nullable list/map if no data
      ...
      questionnaireAnswers: null, // Initialize to null
      personalityAssessmentResults: null, // Initialize to null
      ...
      // Deprecated fields should also be initialized to null
      fullName: null,
      gender: null,
      addressZip: null,
      interests: null,
